    ...
    if len(detections) > 0:
        for k, d in enumerate(detections):
            ...
            if (X_left_crop >= 0 and Y_left_crop >= 0 and X_right_crop < w and Y_right_crop < h):
                ...
                images_total_array.append(mouth_gray)
                cv2.imwrite(mouth_destination_path + '/' + 'frame' + '_' + str(counter) + '.png', mouth_gray)
                print("The cropped mouth is detected ...")
                activation.append(1)
            elif (X_left_crop < 0 or Y_left_crop < 0 or X_right_crop >= w or Y_right_crop >= h):
        ...
        add_image = images_total_array[len(images_total_array)-1]
        cv2.imwrite(mouth_destination_path + '/' + 'frame' + '_' + str(counter) + '.png', add_image)
        images_total_array.append(add_image)
        activation.append(0)
